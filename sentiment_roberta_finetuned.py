...
#######################################
# Initialize Fine-Tuned RoBERTa for Sentiment Analysis
#######################################

# Initialize tqdm for pandas
tqdm.pandas()

print("\nInitializing the fine-tuned RoBERTa model for sentiment analysis.")
# Load the fine-tuned RoBERTa model and tokenizer
model_path = "./roberta-finetuned-sentiment"  # Path to your fine-tuned model
sentiment_analyzer = pipeline("text-classification", model=model_path, tokenizer=model_path, device=device)
tokenizer = AutoTokenizer.from_pretrained(model_path)

MAX_LENGTH = 128  # Adjusted for efficiency with Reddit comments
...
def map_sentiment_to_score(label, score):
    ...
    Assumes sentiment labels are consistent with your fine-tuned model.
    """
    if label.lower() == "positive" or label == "LABEL_2":  # Positive sentiment
        return score  # Score is already in [0, 1]
    elif label.lower() == "negative" or label == "LABEL_0":  # Negative sentiment
        return -score  # Negate the score to map it to [-1, 0]
    elif label.lower() == "neutral" or label == "LABEL_1":  # Neutral sentiment
...
print("\nRunning sentiment analysis on the cleaned dataset...")
...
sentiment_file_path = "data/roberta_finetuned_sentiment_wallstreetbets_data.csv"
...
output_path = "results/sentiment_scores_finetuned_RoBERTa.png"
